...
class Obj {
	...
	move () {
		...
		if ((newposition.x + this.size > windowWidth) || (newposition.x - this.size < 0)) {
			this.movement.x *= -1;
			newposition = p5.Vector.add(this.movement, this.position);
			}
		if ((newposition.y + this.size > windowHeight) || (newposition.y - this.size < 0)) {
			this.movement.y *= -1;
			newposition = p5.Vector.add(this.movement, this.position);
			}
    ...
    if (this.position.x + this.size > windowWidth) this.position.x = windowWidth - this.size;
    if (this.position.y + this.size > windowHeight) this.position.y = windowHeight - this.size;
...
class Food extends Obj {
	...
	live () {
		...
		//var score = (windowHeight - this.position.y);
		var foodSize = foodDensity[round(this.position.x / 10) * ((windowWidth / 10 >> 0) + 1) + round(this.position.y / 10)];
...
class Critter extends Obj {
	...
	draw () {
		if (this.eatenTimer <= 0) fill ('red'); else fill ('green');
		super.draw();
	}
